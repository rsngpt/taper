...
- **No fabricated numbers, anywhere** — code comments, commit messages, README, docs site. Every performance claim in this repo was produced by `./run_benchmark.sh` or the `ConstrainedHeapTest` unit test. If you change parsing/queue internals in a way that could affect the benchmark table in [README.md](README.md) or [docs/index.html](docs/index.html), rerun the harness and update both with the *actual* new numbers — do not hand-edit the table.
